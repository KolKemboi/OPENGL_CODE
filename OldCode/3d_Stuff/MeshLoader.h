...
struct Mesh {
  ...
  Mesh()
      : m_VertexArray(0), m_VertexBuffer(0), m_IndexBuffer(0), m_Shader(),
        m_Transform(glm::vec3(0.0f), glm::vec3(1.0f), glm::vec3(0.0f)),
        m_indexCount(0) {}

  Mesh(u_int vao, u_int vbo, u_int ibo, ModelShader shader, u_int indexCount,
       Transform transform)
      : m_VertexArray(vao), m_VertexBuffer(vbo), m_IndexBuffer(ibo),
        m_Shader(shader), m_Transform(transform), m_indexCount(indexCount) {}
};

Mesh mesh_creator(float *vertices, GLsizei vertexCount, u_int *indices,
                  u_int indexCount, glm::vec3 pos = glm::vec3(0.0f, 0.0f, 0.0f),
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(u_int) * indexCount, indices,
               GL_STATIC_DRAW);
